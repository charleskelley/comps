...
1. Exact matching (exact) - a complete cross of the covariates is used to
...
3. Nearest neighbor (nearest) - performs greedy nearest neighbor matching. A
...
4. Cardinality (cardinality) matching and other forms of matching - using
...
5. Optimal full matching (full) - a form of subclassification wherein all
...
6. Genetic matching (genetic) - a form of nearest neighbor matching where
...
class Method:
    """
    Class with eligible matching methods.

    Attributes:
        distance: A ``comps.match.distance.base.Distance`` instance that is used
            to calculate propensity score and covariate distances for matching
            methods that utilize a distance measure.
    """
    pass
