...
class Distances:
    """
    Standardized dataclass output object to hold all data from a propensity
    ...
    Attributes:
        ...
        values: Matrix of pairwise distances between each target observation
        ...
        target: Name of column in data that is numeric binary indicator
            for target observations where 1 indicates observations
            belong to the target class. For propensity score distance
            classifier algorithms model, the model is fit to predict
            probability of an observation belonging to the target class.
        ...
        rows: Dictionary of class keys 'target' and 'non_target' mapped to
            observation row numbers in the input DataFrame used for distance
            value calculations.

        model: Fitted classifier model instance if a propensity score distances
            algorithm was used.

        propensities: Dictionary of class keys 'target' and 'non_target' mapped
            to observation propensity scores in the same sequences as the row
            numbers mapped in the rows attribute.

        uids: Dictionary of class keys 'target' and 'non_target' mapped to
            observation unique IDs in the same sequences as the row numbers
            mapped in the rows attribute.
    """
